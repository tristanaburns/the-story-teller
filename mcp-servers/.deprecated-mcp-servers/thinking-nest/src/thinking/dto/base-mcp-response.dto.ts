...
export class BaseMCPResponseDto {
  ...
  @ApiProperty({ description: 'Action performed', example: 'analyze' })
  action: string;

  @ApiProperty({ description: 'Status of the operation', example: 'success', enum: ['success', 'error'] })
  status: 'success' | 'error';

  @ApiProperty({ description: 'Error message (if status is error)', example: 'Thinking process not found', required: false })
  error?: string;
